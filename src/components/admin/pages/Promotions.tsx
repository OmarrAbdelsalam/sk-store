...
export default function PromotionsPage() {
  ...
  /**
   * Single-use codes generated by the cart follow-up carry this prefix. They
   * belong to one customer and expire on their own, so they are not campaigns
   * and shouldn't be treated as any.
   */
  const isAutoRecoveryCode = (code: string) => /^BACK\d+-/i.test(String(code || ""));
    ...
    if (activeTab === 'discount_codes') return (
      <>
        <td className="px-6 py-4">
          <span className="font-bold text-gray-900 px-2 py-1 bg-gray-100 rounded-md tracking-wider">{item.name}</span>
          {/* Codes minted automatically for one abandoned cart. Marked so
              nobody mistakes them for a campaign and deletes them — or edits
              one thinking it applies to everybody. */}
          {isAutoRecoveryCode(item.name) && (
            <span className="ms-2 text-[10px] px-1.5 py-0.5 rounded bg-amber-50 text-amber-700 align-middle whitespace-nowrap">
              auto · cart recovery
            </span>
          )}
        </td>
